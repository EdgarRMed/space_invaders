...
def show_math(x, y):
    operation = font_math_operation.render(
        str(operator_1) + " x " + str(operator_2), True, (255, 255, 255)
    )
...
def game_over_text():
    ...
    restart_text = game_over_font.render(
        "Presiona R para reiniciar", True, (255, 255, 255)
    )
    screen.blit(over_text, (150, 240))
    screen.blit(restart_text, (0, 310))


def pause_text():
    pause_text_message = pause_font.render("P A U S A", True, (255, 255, 255))
...
for i in range(number_enemies):
    ...
    monster_result.append(font_math.render(str(monster_value), True, (153, 0, 0)))


def monster(x, y, i):
    screen.blit(monster_img[i], (x, y))
    screen.blit(monster_result[i], (x + 20, y + 40))
...
while running_game:
    ...
    if not pause_status:
        ...
        for i in range(number_enemies):
            ...
            # Define if the result is correct
            if next_math:
                monster_result.insert(
                    correct_answer_monster_pos,
                    font_math.render(str(operator_1 * operator_2), True, (153, 0, 0)),
                )
                monster_values.insert(
                    correct_answer_monster_pos, operator_1 * operator_2
                )
                ...
                if correct_answer_monster_pos < number_enemies:
                    ...
                    monster_result.insert(
                        correct_answer_monster_pos,
                        font_math.render(
                            str(operator_1 * operator_2), True, (153, 0, 0)
                        ),
                    )
                    monster_values.insert(
                        correct_answer_monster_pos, operator_1 * operator_2
